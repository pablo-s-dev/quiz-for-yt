# Release Notes - Version 1.3.0

**Release Date**: May 3, 2026

## 🎉 What's New in 1.3.0

### 📚 Complete Documentation
We've added comprehensive documentation to help you get the most out of Quiz for YouTube:
- Quick Start Guide for new users
- Detailed Installation Guide
- FAQ with common questions
- Contributing guidelines for developers
- Privacy Policy
- And much more!

### 🏗️ Build System
- Added automated build script (`npm run build`)
- Cross-platform support (Windows, Mac, Linux)
- Easier for developers to contribute

### 🎨 Branding Updates
- Official extension name: "Quiz for YouTube"
- Cleaner, more professional branding
- Added creator credit in popup

### ⚡ Performance Improvements
- **Faster quiz generation**: Reduced ChatGPT mode startup time by 75%
- ChatGPT extended mode removal now takes 1.5s instead of 6s
- Smoother overall experience

### 🔧 Under the Hood
- Better code organization
- Improved error handling
- Enhanced reliability

## 🔗 Links

- **Chrome Web Store**: https://chromewebstore.google.com/detail/comfpjldlolnaalknabmalgfdjfghedi
- **Creator Website**: https://pablosan.netlify.app
- **Documentation**: See README.md in the extension folder

## 📝 Full Changelog

See [CHANGELOG.md](CHANGELOG.md) for complete version history.

## 🙏 Thank You!

Thank you for using Quiz for YouTube! Your feedback helps make this extension better.

If you enjoy the extension, please:
- ⭐ Rate it on the Chrome Web Store
- 📝 Leave a review
- 🐛 Report bugs on GitHub
- 💡 Suggest features

---

**Created by PabloS** | https://pablosan.netlify.app
